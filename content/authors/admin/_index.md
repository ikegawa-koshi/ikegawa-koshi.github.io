...
2019年より株式会社日立製作所にてブロックチェーン / Web3領域の研究開発に従事。
入社当初は、ブロックチェーン技術と高信頼実行環境（TEE：Trusted Execution Environment）を組み合わせた、医療・公共分野における機密情報管理の研究に取り組んだ。
近年は自己主権型アイデンティティ（SSI：Self-Sovereign Identity）を主な研究テーマとし、検証可能なデジタル証明書（VC：Verifiable Credentials）を多様なビジネスサービスへ適用するための技術開発を進めている。
また、OSS（Open Source Software）コミュニティへの貢献として、ソフトウェアパッチの提供、技術文書の翻訳、国際カンファレンスでの登壇発表を行っている。
